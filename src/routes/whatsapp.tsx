...
export const Route = createFileRoute("/whatsapp")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Real WhatsApp conversations for every overdue client, synced live from Twilio via n8n.",
      },
...
function useWaConversations() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        conversations: (smsRes.conversations ?? []) as WaConversation[],
...
function WhatsAppInbox() {
  const { data, isLoading, error } = useWaConversations();
  ...
  const activePromise = activeItem
    ? latestForPhone(promises, digits(activeItem.client_id), "whatsapp")
    : undefined;
  const activeEscalation = activeItem
    ? latestForPhone(escalations, digits(activeItem.client_id), "whatsapp")
    : undefined;
  ...
  return (
    <AppShell title="Logs" subtitle="WhatsApp conversations · Twilio · live from n8n">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Promises to pay" value={String(promiseCount)} hint="Confirmed on WhatsApp" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-sm">
            ...
            <tbody className="divide-y divide-border">
              {conversations.map((c) => {
                const promise = latestForPhone(promises, digits(c.client_id), "whatsapp");
                const escalation = latestForPhone(escalations, digits(c.client_id), "whatsapp");
